<template>
  <a-layout class="layout">
    ...
    <template #icon><UserOutlined /></template>
      ...
      <a-button @click="customer">咨询客服</a-button>
      ...
      <span v-for="item,index in msglist">
        ...
        <a :href="'/talk_one?uid='+item.uid+'&username='+item.username">{{ item.username }}</a> 说:{{ item.msg }}  <br /><br />
 ...
 data() {
  ...
  methods:{
    ...
    customer:function(){
          ...
          this.myaxios("http://localhost:5000/customer/","get").then(data => {
                  ...
                  window.location.href = "/talk_one?uid="+data.uid+"&username="+data.username;
    ...
    send:function(){
        ...
        this.myaxios("http://localhost:5000/talk/","post",{"msg":this.msg}).then(data => {
    ...
    init_websocket:function(){
          ...
          ws.onmessage = function(evt){
                ...
                try {
                  ...
                  that.msglist.push({"username":msg["username"],"msg":msg["msg"],"uid":msg["uid"]});
      ...
      axios_upload({
          ...
          data:data
            ...
            if(data.data.errcode == 0){
                  ...
                  this.src = "http://localhost:5000/static/" + data.data.filename;

                  // 存储用户头像的信息

                  localStorage.setItem("avatar",this.src);
